...
async function doExport() {
  if (!cases.length) { alert('No test cases to export.'); return; }
  try {
    const res = await fetch(BACKEND + '/export', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cases, filename: (fileTitle || 'testcases') + '_testcases.xlsx', title: fileTitle }),
    });
    if (!res.ok) throw new Error('Export failed: ' + res.status);
    const blob = await res.blob();
    const a = Object.assign(document.createElement('a'), {
      href: URL.createObjectURL(blob),
      download: (fileTitle || 'testcases') + '_testcases.xlsx',
    });
    a.click();
