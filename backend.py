...
# Tries known local LLM endpoints in order and picks the first one responding.
# Set LLM_BASE_URL env var to skip auto-detection and use a specific URL.
_LLM_CANDIDATES = [
    ("http://localhost:11434/v1",  "Ollama (OpenAI-compat)"),
    ("http://localhost:1234/v1",   "LM Studio"),
    ("http://localhost:1337/v1",   "Jan"),
    ("http://localhost:8080/v1",   "llama.cpp"),
    ("http://localhost:5001/v1",   "text-generation-webui"),
]

def _auto_detect_llm():
    """Try each candidate URL and return the first that responds to /models."""
    global LLM_BASE_URL, LLM_MODEL
    # If user explicitly set env var, respect it
    if os.getenv("LLM_BASE_URL"):
        return
    for url, name in _LLM_CANDIDATES:
        try:
            r = requests.get(f"{url}/models", timeout=3)
            if r.status_code == 200:
                data  = r.json()
                LLM_BASE_URL = url
                # Auto-pick first available model if not set by env
                if not os.getenv("LLM_MODEL"):
                    models = data.get("data", [])
                    if models:
                        LLM_MODEL = models[0].get("id", LLM_MODEL)
                print(f"[LLM] Auto-detected: {name} at {url}  model={LLM_MODEL}")
                return
        except Exception:
            continue
    print(f"[LLM] Warning: no local LLM found. Defaulting to {LLM_BASE_URL}")
    print("[LLM] Start Ollama with: ollama serve && ollama pull llama3")

def _call_llm_chat(messages, temperature=0.3, max_tokens=4096):
    """
    Unified LLM chat call — handles both OpenAI-compat (/v1/chat/completions)
    and Ollama native (/api/chat) automatically.
    Retries once if we get a 404 (wrong path).
    """
    payload = {
        "model": LLM_MODEL,
        "messages": messages,
    ...
    # If 404, try Ollama native API as fallback
    if resp.status_code == 404:
        ...
        resp2 = requests.post(ollama_url, json=ollama_payload, timeout=LLM_TIMEOUT)
        if resp2.status_code == 200:
            # Normalize Ollama native response to OpenAI format
            d = resp2.json()
            content = d.get("message", {}).get("content", "")
            return content.strip()
        # Both failed — raise the original 404 error with helpful message
        resp.raise_for_status()
...
def call_llm(puml, parsed, knowledge=None):
    ...
    raw = _call_llm_chat(messages, temperature=0.3, max_tokens=4096)
...
def call_llm_diagram(description, diagram_type):
    ...
    raw = _call_llm_chat(messages, temperature=0.4, max_tokens=1024)
...
def llm_config():
    """Return current LLM configuration for the Settings view."""
    return jsonify({
        "llm_url":   LLM_BASE_URL,
        "llm_model": LLM_MODEL,
        "timeout":   LLM_TIMEOUT,
        "candidates": [{"url": u, "name": n} for u, n in _LLM_CANDIDATES],
    })
...
Return ONLY the PlantUML code. No explanation, no markdown fences."""
    ...
    try:
        # Vision requires multimodal payload — cannot use _call_llm_chat wrapper
        # Must go directly to /v1/chat/completions with image_url content
        vision_url = f"{LLM_BASE_URL}/chat/completions"
        payload = {
            "model": LLM_MODEL,
            "messages": [
                {
        ...
        resp = requests.post(vision_url, json=payload, timeout=LLM_TIMEOUT)
        if resp.status_code == 404:
            raise RuntimeError(
                f"Vision endpoint not found at {vision_url}. "
                "Your LLM must support vision. Try: ollama pull llava or ollama pull llama3.2-vision"
            )
        resp.raise_for_status()
...
Return ONLY the explanation text, no markdown, no bullet points."""
    ...
    try:
        ...
        explanation = _call_llm_chat(messages, temperature=0.4, max_tokens=400)
...
if __name__ == "__main__":
    ...
    print()
    print("  LLM providers checked:")
    for url, name in _LLM_CANDIDATES:
        marker = " ← active" if url == LLM_BASE_URL else ""
        print(f"    {name:30s}  {url}{marker}")
    print("=" * 60)
    print()
